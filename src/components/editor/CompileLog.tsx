 ...
 *
 * Tabs: Errors | Warnings | Bad Boxes | Missing Refs | Raw Log
 * Each log item is individually expandable to show context.
 */
...
const TABS: TabDef[] = [
  { id: 'errors', label: 'Errors', countKey: 'errorsCount', color: 'text-danger', badgeClass: 'compile-log-badge-error' },
  { id: 'warnings', label: 'Warnings', countKey: 'warningsCount', color: 'text-warning', badgeClass: 'compile-log-badge-warning' },
  { id: 'badboxes', label: 'Bad Boxes', countKey: 'badboxesCount', color: 'text-content-secondary', badgeClass: 'compile-log-badge-muted' },
  { id: 'missingRefs', label: 'Missing Refs', countKey: 'missingRefsCount', color: 'text-content-secondary', badgeClass: 'compile-log-badge-muted' },
  { id: 'rawLog', label: 'Raw Log', countKey: null, color: 'text-content-secondary', badgeClass: '' },
]
      ...
      >
        <svg
          ...
          className={`shrink-0 transition-transform duration-150 ${
            hasContext ? (expanded ? 'rotate-90' : '') : 'opacity-0'
          }`}
        >
        ...
        {item.package && (
          <span className="compile-log-item-package">{item.package}</span>
        )}

        {item.file && (
          <span className="compile-log-item-file">{item.file}</span>
        )}
        ...
        {item.line != null && (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation()
              onJumpToLine?.(item.line!, item.file)
            }}
...
export function CompileLog({ compilationLog, onJumpToLine, forceOpen, height, onOpenChange }: CompileLogProps) {
  ...
  const [activeTab, setActiveTab] = useState<LogCategory>('errors')
  ...
  useEffect(() => {
    if (forceOpen && totalIssues > 0) {
      setIsOpen(true)
      onOpenChange?.(true)
      if (log.summary.errorsCount > 0) setActiveTab('errors')
      else if (log.summary.warningsCount > 0) setActiveTab('warnings')
      else if (log.summary.badboxesCount > 0) setActiveTab('badboxes')
      else setActiveTab('missingRefs')
    }
  }, [forceOpen, totalIssues, log.summary, onOpenChange])

  const handleJump = useCallback((line: number, file?: string) => {
