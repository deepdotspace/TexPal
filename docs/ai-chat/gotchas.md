...
## 5. `useChat` needs dynamic body AND dynamic fetch

`useChat({ body })` is reactive — when the `body` prop changes (e.g. user switches files, so `activeFilePath` changes), the next send uses the new body. Good.

For auth, `useChat({ headers })` only accepts a static object. To attach a fresh `Authorization: Bearer <JWT>` on every send, wrap `fetch`:

```ts
useChat({
  api: '/api/ai/chat',
  body: { documentId, activeFilePath },
...
## 6. Reset `useChat` messages on document switch

The chat is scoped per-document. When `documentId` changes, call `setMessages([])`:
